...
class dataset(Dataset):
    ...
    def __init__(self):
        data = pd.read_csv('./2_square.csv',names=['max','mean','root_mean_square','standard_deviation','y'])
...
#main函数内使用
def pred_square(data, weight_path):
    pred_list=[]
    model = Model()
    model.load_state_dict(torch.load(weight_path))
    model.eval()
    ...
    for batch, X in enumerate(data):
        ...
        # 进行预测，也就是做了一次前向传播
        y_pred = model(tensorX[-4:])
        y_pred = y_pred.data.item()
        if y_pred >= 0.5:
            pred_list.append(X[:2])
    end_time = time.time()
    return pred_list, end_time - start_time
...
if __name__ == '__main__':
    data = dataset()
    model = Model()
    model.load_state_dict(torch.load('./SIweight_square.pth'))
    model.eval()
