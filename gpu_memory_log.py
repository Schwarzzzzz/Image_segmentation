...
def gpu_memory_log(gpu_log_file="gpu_mem.log", device=0):
    ...
    with open(gpu_log_file, 'a+') as f:
        ...
        memory_allocated = float(torch.cuda.memory_allocated()/1024**2)
        write_str = "memory_allocated:%f Mb" % memory_allocated
        # print(write_str)
        max_memory_allocated = float(torch.cuda.max_memory_allocated()/1024**2)
        write_str = "max_memory_allocated:%f Mb" % max_memory_allocated
        # print(write_str)
        memory_cached = float(torch.cuda.memory_cached()/1024**2)
        write_str = "memory_cached:%f Mb" % memory_cached
        # print(write_str)
        max_memory_cached = float(torch.cuda.max_memory_cached()/1024**2)
        write_str = "max_memory_cached:%f Mb" % max_memory_cached
        # print(write_str)
        Total_Memory = float(meminfo.total/1024**2)
        write_str = "Total Memory:%f Mb" % Total_Memory
        # print(write_str)

        return memory_allocated, max_memory_allocated, memory_cached, max_memory_cached, Total_Memory
